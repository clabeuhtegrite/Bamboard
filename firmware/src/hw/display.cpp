...
//
// The pin map (config.h pins::) targets the JC4827W543**C** revision and
// matches the validated Arduino_GFX reference for this board. If the screen is
// black or garbled, re-check the silkscreen next to the FPC connector — other
// revisions and the 800×480 sibling use a different controller/mapping.
...
// ---------- Panel (NV3041A / QSPI) + touch (GT911 / I²C) -------------------
//
// Constructed once in begin() rather than as load-time globals so there's no
// static-init ordering surprise before the Arduino core is up.
...
static void flush_cb(lv_disp_drv_t* drv, const lv_area_t* area, lv_color_t* buf) {
    ...
    // LV_COLOR_16_SWAP is 0, so lv_color_t is a native RGB565 word — exactly
    // what draw16bitRGBBitmap expects.
...
bool Display::begin() {
    ...
    // A reset pulse with the controller's I²C bus already up; the GT911 latches
    // its slave address (0x5D here) during this window. Touch failing to come
    // up must NOT fail begin() — a desk monitor is still useful read-only.
    ...
    // the buffer out itself over QSPI, so it needs no DMA-capable / PSRAM region
    // — keeping it off PSRAM removes that dependency from the boot path.
